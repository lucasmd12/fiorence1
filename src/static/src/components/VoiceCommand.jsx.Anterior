...
const VoiceCommand = ({ context, onTransactionAdded }) => {
  ...
  // CIRURGIA 2: Novo estado para rastrear o último campo preenchido
  ...
  const handleMissingFieldResponse = async (response) => {
    ...
    if (remainingMissing.length > 0) {
      ...
      const question = CommandMapper.generateQuestion(nextField, updatedTransaction, { lastFieldFilled: currentField });
      addToConversation(setConversation, 'assistant', question);
      speakText(question);
    } else {
